    ...
    draw_nodes(visible_nodes, t){
        ...
        const node_e = node
            ...
            .on("mouseover", (d,i,c) => {
                ...
                select(c[i]).selectAll("circle").transition(300)
                    .attr("r", this.r+2);
            })
            .on("mouseout", (d,i,c) => {
            ...
            .call(drag()
                ...
                .on("drag", (d, i, c)=> {
                    ...
                    if (d.has_loaded_leaves)
                        select(c[i]).attr("transform",
                            d => "translate(" + event.x + "," + d.y + ")"
                        );
                })
                .on("end", (d, i, c) => {
                    if (d.has_loaded_leaves) {
                        ...
                        select(c[i]).transition(t).attr("transform",
                            d => "translate(" + d.x + "," + d.y + ")");
                    }
                })
            )
...
class GenomePropertiesTaxonomy {
    ...
    update_tree(time=0, cell_side=null){
        ...
        this.orders = {
            tax_id: range(ol).sort((a, b) => leaves[a].data.taxId - leaves[b].data.taxId),
            org_name: range(ol).sort((a, b) => leaves[a].data.species - leaves[b].data.species),
            tree: range(ol).sort((a, b) => leaves[a].data.lineage - leaves[b].data.lineage),
        };
        ...
        select(".taxon_tree").attr("transform",
            "translate(0 ,"
            +(-this.height+20)
            +")");
...
class GenomePropertiesController {
    ...
    draw_legends(total={YES: 0, NO: 0, PARTIAL: 0}) {
        const legend_item = this.legends_component.selectAll("li")
            .data(entries(total).sort((a,b)=>a.key>b.key?-1:1), d=>d.key);
...
class GenomePropertiesViewer {
    ...
    constructor({
        ...
        if(whitelist_path){
            json(whitelist_path, (data)=>this.whitelist = data);
        }
        this.svg = select(element_selector).append("svg")
    ...
    refresh_size(){
        ...
        const rect = select(this.options.element_selector).node().getBoundingClientRect();
            ...
            .call(drag() // Window panning.
                ...
                .on("drag", function(_this) {
                    return function() {
                        ...
                        let dy = -event.y * th / (_this.options.height - this.getAttribute("height"));
                        dy = Math.max(
                            Math.min(dy, 0),
    ...
    load_genome_properties_file(tax_id) {
        if (this.organisms.indexOf(Number(tax_id)) !== -1)
            return;
        text(`${this.options.server}${tax_id}`)
            .get((error, text) => {
                if (error) throw error;
    ...
    update_total_per_organism_panel(time=0) {
        ...
        const cells_t = this.total_g.selectAll(".total_cell_org")
            .data(entries(this.organism_totals)
        ...
        if(this.legend_filters){
            for (let x in this.legend_filters){
                if (this.legend_filters[x]=="∀")
                    this.props = this.props.filter(e=>{
                        const values =entries(e.values).filter(e=>e.key!="TOTAL");
    ...
    update_viewer(zoom=false, time=0) {
        this.props = values(this.data).filter(e=>{
        ...
        if (!zoom){
            dy = -Math.floor(
                    select(".gpv-rows-group").attr("transform").match(/translate\((.*),(.*)\)/)[2] / this.y(1)
                ) - 2;
            dy = dy < 0 ? 0 : dy;
            this.current_dy = dy;
        }
        ...
        const t = transition().duration(time);
    ...
    update_row(r,i,c) {
        ...
        const gps = select(c[i]).selectAll(".top_level_gp")
        ...
        let cells = select(c[i]).selectAll(".cell")
        ...
        const mouseover = (p,i,c)=> {
            ...
            const data = select(c[i].parentNode).data();
        ...
        const cells_t = select(c[i]).selectAll(".total_cell")
            .data(["TOTAL"], d=>d);
        ...
        console.log(entries(r.values["TOTAL"]));
        const arcs = select(c[i]).selectAll(".total_cell").selectAll(".arc")
            .data(pie$$(entries(r.values["TOTAL"])), d=>d.data.key);
