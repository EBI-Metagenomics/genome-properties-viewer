...
class GenomePropertiesViewer {
    ...
    constructor({margin={top: 80, right: 0, bottom: 10, left: 80},
        ...
        this.y = linear().range([0, height]);
        // this.z = d3.scaleLinear().domain([0, 4]).clamp(true);
        this.c = ordinal(schemeCategory20c).domain(["YES", "NO", "PARTIAL"]);
    ...
    load_genome_properties_file(tax_id) {
        text(`../test-files/SUMMARY_FILE_${tax_id}`)
            .get((error, text) => {
    ...
    update_viewer() {
        const props = values(this.data).slice(0,100);
